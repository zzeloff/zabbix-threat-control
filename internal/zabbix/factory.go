...
// New builds a Client for the detected/requested Zabbix version. 7.x and 6.4+
// authenticate with the Authorization: Bearer header (NewV7); 6.0/6.2 and 5.x
// pass the token in the request "auth" field (NewV6). When version is empty it
// is autodetected via apiinfo.version (which needs no authentication).
...
// "username" login parameter (5.x, 6.0, 6.2). 6.4+/7.x and an unknown version
// default to the header-based 7.x client.
